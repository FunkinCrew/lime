	...
	namespace metal
	{
		...
		static uint16_t writeUniformArray(bx::WriterI *_shaderWriter, const UniformArray &uniforms, bool isFragmentShader)
		{
			...
			for (uint16_t ii = 0; ii < count; ++ii)
			{
				...
				BX_TRACE("%s, %s, %d, %d, %d", un.name.c_str(), getUniformTypeName(un.type), un.num, un.regIndex, un.regCount);
			}
			return size;
		}
		...
		{
			return "ios" == _platform ? spirv_cross::CompilerMSL::Options::Platform::iOS : spirv_cross::CompilerMSL::Options::Platform::macOS;
		}
