...
namespace bgfx
{
	namespace hlsl
	{
		typedef HRESULT(WINAPI *PFN_D3D_COMPILE)(LPCVOID pSrcData, SIZE_T SrcDataSize, LPCSTR pSourceName, CONST D3D_SHADER_MACRO *pDefines, ID3DInclude *pInclude, LPCSTR pEntrypoint, LPCSTR pTarget, UINT Flags1, UINT Flags2, ID3DBlob **ppCode, ID3DBlob **ppErrorMsgs);

		typedef HRESULT(WINAPI *PFN_D3D_DISASSEMBLE)(LPCVOID pSrcData, SIZE_T SrcDataSize, UINT Flags, LPCSTR szComments, ID3DBlob **ppDisassembly);

		typedef HRESULT(WINAPI *PFN_D3D_REFLECT)(LPCVOID pSrcData, SIZE_T SrcDataSize, REFIID pInterface, void **ppReflector);

		typedef HRESULT(WINAPI *PFN_D3D_STRIP_SHADER)(LPCVOID pShaderBytecode, SIZE_T BytecodeLength, UINT uStripFlags, ID3DBlob **ppStrippedBlob);
		...
		static const D3DCompiler s_d3dcompiler[] = {
			...
			{"D3DCompiler_47.dll", {0x8d536ca1, 0x0cca, 0x4956, {0xa8, 0x37, 0x78, 0x69, 0x63, 0x75, 0x55, 0x84}}}, {"D3DCompiler_46.dll", {0x0a233719, 0x3960, 0x4578, {0x9d, 0x7c, 0x20, 0x3b, 0x8b, 0x1d, 0x9c, 0xc1}}}, {"D3DCompiler_45.dll", {0x0a233719, 0x3960, 0x4578, {0x9d, 0x7c, 0x20, 0x3b, 0x8b, 0x1d, 0x9c, 0xc1}}},
			{"D3DCompiler_44.dll", {0x0a233719, 0x3960, 0x4578, {0x9d, 0x7c, 0x20, 0x3b, 0x8b, 0x1d, 0x9c, 0xc1}}}, {"D3DCompiler_43.dll", {0x0a233719, 0x3960, 0x4578, {0x9d, 0x7c, 0x20, 0x3b, 0x8b, 0x1d, 0x9c, 0xc1}}},
		};
		...
		const D3DCompiler *load(bx::WriterI *_messageWriter)
		{
			...
			for (uint32_t ii = 0; ii < BX_COUNTOF(s_d3dcompiler); ++ii)
			{
				...
				if (NULL == D3DCompile || NULL == D3DDisassemble || NULL == D3DReflect || NULL == D3DStripShader)
				{
		...
		static const RemapInputSemantic s_remapInputSemantic[bgfx::Attrib::Count + 1] = {
			{bgfx::Attrib::Position, "POSITION", 0},  {bgfx::Attrib::Normal, "NORMAL", 0},		{bgfx::Attrib::Tangent, "TANGENT", 0},		{bgfx::Attrib::Bitangent, "BITANGENT", 0},	{bgfx::Attrib::Color0, "COLOR", 0},			{bgfx::Attrib::Color1, "COLOR", 1},			{bgfx::Attrib::Color2, "COLOR", 2},			{bgfx::Attrib::Color3, "COLOR", 3},			{bgfx::Attrib::Indices, "BLENDINDICES", 0},
			{bgfx::Attrib::Weight, "BLENDWEIGHT", 0}, {bgfx::Attrib::TexCoord0, "TEXCOORD", 0}, {bgfx::Attrib::TexCoord1, "TEXCOORD", 1},	{bgfx::Attrib::TexCoord2, "TEXCOORD", 2},	{bgfx::Attrib::TexCoord3, "TEXCOORD", 3},	{bgfx::Attrib::TexCoord4, "TEXCOORD", 4},	{bgfx::Attrib::TexCoord5, "TEXCOORD", 5},	{bgfx::Attrib::TexCoord6, "TEXCOORD", 6},	{bgfx::Attrib::TexCoord7, "TEXCOORD", 7},
			{bgfx::Attrib::TexCoord8, "TEXCOORD", 8}, {bgfx::Attrib::TexCoord9, "TEXCOORD", 9}, {bgfx::Attrib::TexCoord10, "TEXCOORD", 10}, {bgfx::Attrib::TexCoord11, "TEXCOORD", 11}, {bgfx::Attrib::TexCoord12, "TEXCOORD", 12}, {bgfx::Attrib::TexCoord13, "TEXCOORD", 13}, {bgfx::Attrib::TexCoord14, "TEXCOORD", 14}, {bgfx::Attrib::TexCoord15, "TEXCOORD", 15}, {bgfx::Attrib::Count, "", 0},
		};
		...
		const RemapInputSemantic &findInputSemantic(const char *_name, uint8_t _index)
		{
			for (uint32_t ii = 0; ii < bgfx::Attrib::Count; ++ii)
			{
				...
				if (0 == bx::strCmp(ris.m_name, _name) && ris.m_index == _index)
				{
					return ris;
				}
			}
		...
		static const UniformRemap s_uniformRemap[] = {
			{UniformType::Sampler, D3D_SVC_SCALAR, D3D_SVT_INT, 0, 0},	   {UniformType::Vec4, D3D_SVC_VECTOR, D3D_SVT_FLOAT, 0, 0},		{UniformType::Mat3, D3D_SVC_MATRIX_COLUMNS, D3D_SVT_FLOAT, 3, 3}, {UniformType::Mat4, D3D_SVC_MATRIX_COLUMNS, D3D_SVT_FLOAT, 4, 4},
			{UniformType::Sampler, D3D_SVC_OBJECT, D3D_SVT_SAMPLER, 0, 0}, {UniformType::Sampler, D3D_SVC_OBJECT, D3D_SVT_SAMPLER2D, 0, 0}, {UniformType::Sampler, D3D_SVC_OBJECT, D3D_SVT_SAMPLER3D, 0, 0},  {UniformType::Sampler, D3D_SVC_OBJECT, D3D_SVT_SAMPLERCUBE, 0, 0},
		};
		...
		UniformType::Enum findUniformType(const D3D11_SHADER_TYPE_DESC &constDesc)
		{
			for (uint32_t ii = 0; ii < BX_COUNTOF(s_uniformRemap); ++ii)
			{
				...
				if (remap.paramClass == constDesc.Class && remap.paramType == constDesc.Type)
				{
					...
					if (remap.columns == constDesc.Columns && remap.rows == constDesc.Rows)
					{
						return remap.id;
					}
				}
		...
		bool getReflectionDataD3D11(ID3DBlob *_code, bool _vshader, UniformArray &_uniforms, uint8_t &_numAttrs, uint16_t *_attrs, uint16_t &_size, UniformNameList &unusedUniforms, bx::WriterI *_messageWriter)
		{
			...
			HRESULT hr = D3DReflect(_code->GetBufferPointer(), _code->GetBufferSize(), s_compiler->reflectionGuid, (void **)&reflect);
			...
			if (_vshader) // Only care about input semantic on vertex shaders
			{
				for (uint32_t ii = 0; ii < desc.InputParameters; ++ii)
				{
					...
					BX_TRACE("\t%2d: %s%d, vt %d, ct %d, mask %x, reg %d", ii, spd.SemanticName, spd.SemanticIndex, spd.SystemValueType, spd.ComponentType, spd.Mask, spd.Register);
			...
			for (uint32_t ii = 0, num = bx::min(1, desc.ConstantBuffers); ii < num; ++ii)
			{
				...
				if (SUCCEEDED(hr))
				{
					BX_TRACE("%s, %d, vars %d, size %d", bufferDesc.Name, bufferDesc.Type, bufferDesc.Variables, bufferDesc.Size);
					...
					for (uint32_t jj = 0; jj < bufferDesc.Variables; ++jj)
					{
						...
						if (SUCCEEDED(hr))
						{
							...
							if (SUCCEEDED(hr))
							{
								...
								if (UniformType::Count != uniformType && 0 != (varDesc.uFlags & D3D_SVF_USED))
								{
									Uniform un;
									...
									BX_TRACE("\t%s, %d, size %d, flags 0x%08x, %d (used)", varDesc.Name, varDesc.StartOffset, varDesc.Size, varDesc.uFlags, uniformType);
								}
								else
								{
			...
			for (uint32_t ii = 0; ii < desc.BoundResources; ++ii)
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					{
						BX_TRACE("\t%s, %d, %d, %d", bindDesc.Name, bindDesc.Type, bindDesc.BindPoint, bindDesc.BindCount);
		...
		static bool compile(const Options &_options, uint32_t _version, const std::string &_code, bx::WriterI *_shaderWriter, bx::WriterI *_messageWriter, bool _firstPass)
		{
			...
			HRESULT hr = D3DCompile(_code.c_str(), _code.size(), hlslfp.c_str(), NULL, NULL, "main", profileAndType, flags, 0, &code, &errorMsg);
			if (FAILED(hr) || (werror && NULL != errorMsg))
			{
				...
				bool found = false || 2 == sscanf(log, "(%u,%u", &line, &column) || 2 == sscanf(log, " :%u:%u: ", &line, &column);
...

			{
				...
				for (UniformArray::const_iterator it = uniforms.begin(); it != uniforms.end(); ++it)
				{
					...
					BX_TRACE("%s, %s, %d, %d, %d", un.name.c_str(), getUniformTypeName(UniformType::Enum(un.type & ~kUniformMask)), un.num, un.regIndex, un.regCount);
				}
			}

			{
				ID3DBlob *stripped;
				hr = D3DStripShader(code->GetBufferPointer(), code->GetBufferSize(), D3DCOMPILER_STRIP_REFLECTION_DATA | D3DCOMPILER_STRIP_TEST_BLOBS, &stripped);
			...
			{
				ID3DBlob *disasm;
				D3DDisassemble(code->GetBufferPointer(), code->GetBufferSize(), 0, NULL, &disasm);
