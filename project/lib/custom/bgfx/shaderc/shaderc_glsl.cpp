...
namespace bgfx
{
	namespace glsl
	{
		static bool compile(const Options &_options, uint32_t _version, const std::string &_code, bx::WriterI *_shaderWriter, bx::WriterI *_messageWriter)
		{
			...
			const glslopt_shader_type type = ch == 'f' ? kGlslOptShaderFragment : (ch == 'c' ? kGlslOptShaderCompute : kGlslOptShaderVertex);
			...
			if (!glslopt_get_status(shader))
			{
				...
				bool found = false || 3 == sscanf(log, "%u:%u(%u):", &source, &line, &column) || 2 == sscanf(log, "(%u,%u):", &line, &column);
			...
			if (target != kGlslTargetMetal)
			{
				...
				while (!parse.isEmpty())
				{
					...
					if (!eol.isEmpty())
					{
						...
						if (0 == bx::strCmp(qualifier, "attribute", 9) || 0 == bx::strCmp(qualifier, "varying", 7) || 0 == bx::strCmp(qualifier, "in", 2) || 0 == bx::strCmp(qualifier, "out", 3))
						{
						...
						if (0 == bx::strCmp(qualifier, "flat", 4) || 0 == bx::strCmp(qualifier, "smooth", 6) || 0 == bx::strCmp(qualifier, "noperspective", 13) || 0 == bx::strCmp(qualifier, "centroid", 8))
						{
						...
						if (0 == bx::strCmp(typen, "sampler", 7) || 0 == bx::strCmp(typen, "isampler", 8) || 0 == bx::strCmp(typen, "usampler", 8))
						{
			...
			else
			{
				...
				while (parse.getPtr() < end.getPtr() && !parse.isEmpty())
				{
