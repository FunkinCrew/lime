...
#define SHADERC_CONFIG_HAS_DXC (0 || BX_PLATFORM_WINDOWS || BX_PLATFORM_LINUX)
...
namespace bgfx
{
	class ShaderCWriter : public bx::WriterI
	{
	  public:
		int32_t write(const void *_data, int32_t _size, bx::Error *) override
		{
		...
		const bgfx::Memory *finalize()
		{
			if (m_buffer.empty())
				return nullptr;
			const bgfx::Memory *mem = bgfx::alloc(static_cast<uint32_t>(m_buffer.size()));
			std::memcpy(mem->data, m_buffer.data(), m_buffer.size());
			return mem;
		}

		std::string str() const { return std::string(m_buffer.begin(), m_buffer.end()); }
	...
	constexpr uint16_t kAccessMask = 0 | kAccessRead | kAccessWrite;
	...
	constexpr uint8_t kUniformMask = 0 | kUniformFragmentBit | kUniformSamplerBit | kUniformReadOnlyBit | kUniformCompareBit;
	...
	struct Uniform
	{
		Uniform() : type(UniformType::Count), num(0), regIndex(0), regCount(0), texComponent(0), texDimension(0), texFormat(0) {}
	...
	inline bool compileGLSLShader(const Options &_options, uint32_t _version, const std::string &_code, bx::WriterI *_writer, bx::WriterI *_messages)
	{
		return false;
	}
	inline bool compileHLSLShader(const Options &_options, uint32_t _version, const std::string &_code, bx::WriterI *_writer, bx::WriterI *_messages)
	{
		return false;
	}
	inline bool compileSPIRVShader(const Options &_options, uint32_t _version, const std::string &_code, bx::WriterI *_writer, bx::WriterI *_messages)
	{
		return false;
	}
#else
	...
	inline bool compileMetalShader(const Options &_options, uint32_t _version, const std::string &_code, bx::WriterI *_writer, bx::WriterI *_messages)
	{
		return false;
	}
#endif

	inline bool compileDxilShader(const Options &, uint32_t, const std::string &, bx::WriterI *, bx::WriterI *)
	{
		return false;
	}
	inline bool compileWgslShader(const Options &, uint32_t, const std::string &, bx::WriterI *, bx::WriterI *)
	{
		return false;
	}
	inline bool compilePSSLShader(const Options &, uint32_t, const std::string &, bx::WriterI *, bx::WriterI *)
	{
		return false;
	}
	inline const char *getPsslPreamble()
	{
		return "";
	}

	bool compileShader(const char *_varying, const char *_outputFilePath, char *_shader, uint32_t _shaderLen, const Options &_options, bx::WriterI *_shaderWriter, bx::WriterI *_messageWriter);
