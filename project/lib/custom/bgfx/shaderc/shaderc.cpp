...
#define fatal fatal_shaderc
#define trace trace_shaderc
...
namespace bgfx
{
	...
	static const char *s_shadingLangName[] = {"OpenGL ES Shading Language / WebGL (ESSL)",
	...
	static const Profile s_profiles[] = {
		{ShadingLang::ESSL, 100, "100_es"},		  {ShadingLang::ESSL, 300, "300_es"},		{ShadingLang::ESSL, 310, "310_es"},		  {ShadingLang::ESSL, 320, "320_es"}, {ShadingLang::HLSL, 400, "s_4_0"},   {ShadingLang::HLSL, 500, "s_5_0"},		 {ShadingLang::Dxil, 600, "s_6_0"},		   {ShadingLang::Dxil, 610, "s_6_1"},		 {ShadingLang::Dxil, 620, "s_6_2"},		   {ShadingLang::Dxil, 630, "s_6_3"},		 {ShadingLang::Dxil, 640, "s_6_4"},		   {ShadingLang::Dxil, 650, "s_6_5"},
		{ShadingLang::Dxil, 660, "s_6_6"},		  {ShadingLang::Dxil, 670, "s_6_7"},		{ShadingLang::Dxil, 680, "s_6_8"},		  {ShadingLang::Dxil, 690, "s_6_9"},  {ShadingLang::Metal, 1210, "metal"}, {ShadingLang::Metal, 1000, "metal10-10"}, {ShadingLang::Metal, 1110, "metal11-10"}, {ShadingLang::Metal, 1210, "metal12-10"}, {ShadingLang::Metal, 2011, "metal20-11"}, {ShadingLang::Metal, 2111, "metal21-11"}, {ShadingLang::Metal, 2211, "metal22-11"}, {ShadingLang::Metal, 2314, "metal23-14"},
		{ShadingLang::Metal, 2414, "metal24-14"}, {ShadingLang::Metal, 3014, "metal30-14"}, {ShadingLang::Metal, 3114, "metal31-14"}, {ShadingLang::PSSL, 1000, "pssl"},  {ShadingLang::SpirV, 1010, "spirv"}, {ShadingLang::SpirV, 1010, "spirv10-10"}, {ShadingLang::SpirV, 1311, "spirv13-11"}, {ShadingLang::SpirV, 1411, "spirv14-11"}, {ShadingLang::SpirV, 1512, "spirv15-12"}, {ShadingLang::SpirV, 1613, "spirv16-13"}, {ShadingLang::GLSL, 120, "120"},		   {ShadingLang::GLSL, 130, "130"},
		{ShadingLang::GLSL, 140, "140"},		  {ShadingLang::GLSL, 150, "150"},			{ShadingLang::GLSL, 330, "330"},		  {ShadingLang::GLSL, 400, "400"},	  {ShadingLang::GLSL, 410, "410"},	   {ShadingLang::GLSL, 420, "420"},			 {ShadingLang::GLSL, 430, "430"},		   {ShadingLang::GLSL, 440, "440"},			 {ShadingLang::WGSL, 1010, "wgsl"},
	};
	...
	static const char *s_EXT_shader_texture_lod[] = {"texture2DLod", "texture2DProjLod", "textureCubeLod", "texture2DGrad", "texture2DProjGrad", "textureCubeGrad", NULL};

	static const char *s_EXT_shadow_samplers[] = {"shadow2D", "shadow2DProj", "sampler2DShadow", NULL};

	static const char *s_OES_standard_derivatives[] = {"dFdx", "dFdy", "fwidth", NULL};

	static const char *s_OES_texture_3D[] = {"texture3D", "texture3DProj", "texture3DLod", "texture3DProjLod", NULL};

	static const char *s_EXT_gpu_shader4[] = {"gl_VertexID", "gl_InstanceID", "texture2DLodOffset", NULL};
	...
	static const char *s_ARB_shader_viewport_layer_array[] = {"gl_ViewportIndex", "gl_Layer", NULL};

	static const char *s_ARB_gpu_shader5[] = {"bitfieldReverse", "floatBitsToInt", "floatBitsToUint", "intBitsToFloat", "uintBitsToFloat", NULL};

	static const char *s_ARB_shading_language_packing[] = {"packHalf2x16", "unpackHalf2x16", NULL};

	static const char *s_130[] = {"uint", "uint2", "uint3", "uint4", "isampler2D", "usampler2D", "isampler3D", "usampler3D", "isamplerCube", "usamplerCube", "textureSize", NULL};

	static const char *s_textureArray[] = {"sampler2DArray", "texture2DArray", "texture2DArrayLod", "shadow2DArray", NULL};

	static const char *s_ARB_texture_multisample[] = {"sampler2DMS", "isampler2DMS", "usampler2DMS", NULL};

	static const char *s_texelFetch[] = {"texelFetch", "texelFetchOffset", NULL};

	static const char *s_bitsToEncoders[] = {"floatBitsToUint", "floatBitsToInt", "intBitsToFloat", "uintBitsToFloat", NULL};

	static const char *s_integerVecs[] = {"ivec2", "uvec2", "ivec3", "uvec3", "ivec4", "uvec4", NULL};

	static const char *s_allowedVertexShaderInputs[] = {"a_position",	"a_normal",		"a_tangent",	"a_bitangent", "a_color0", "a_color1", "a_color2", "a_color3", "a_indices", "a_weight", "a_texcoord0", "a_texcoord1", "a_texcoord2", "a_texcoord3", "a_texcoord4", "a_texcoord5", "a_texcoord6", "a_texcoord7", "a_texcoord8", "a_texcoord9", "a_texcoord10", "a_texcoord11", "a_texcoord12",
														"a_texcoord13", "a_texcoord14", "a_texcoord15", "i_data0",	   "i_data1",  "i_data2",  "i_data3",  "i_data4",  "i_data5",	"i_data6",	"i_data7",	   "i_data8",	  "i_data9",	 "i_data10",	"i_data11",	   "i_data12",	  "i_data13",	 "i_data14",	"i_data15",	   NULL};
	...
	Options::Options() : shaderType(' '), disasm(false), raw(false), preprocessOnly(false), keepComments(false), depends(false), debugInformation(false), avoidFlowControl(false), noPreshader(false), partialPrecision(false), preferFlowControl(false), backwardsCompatibility(false), warningsAreErrors(false), keepIntermediate(false), optimize(false), optimizationLevel(3), fileOpen(NULL), fileOpenUserData(NULL) {}
	...
	void Options::dump()
	{
		BX_TRACE("Options:\n"
				 ...
				 ,
				 shaderType, platform.c_str(), profile.c_str(), inputFilePath.c_str(), outputFilePath.c_str(), disasm ? "true" : "false", raw ? "true" : "false", preprocessOnly ? "true" : "false", keepComments ? "true" : "false", depends ? "true" : "false", debugInformation ? "true" : "false", avoidFlowControl ? "true" : "false", noPreshader ? "true" : "false", partialPrecision ? "true" : "false", preferFlowControl ? "true" : "false", backwardsCompatibility ? "true" : "false",
				 warningsAreErrors ? "true" : "false", keepIntermediate ? "true" : "false", optimize ? "true" : "false", optimizationLevel);
	...
	const char *s_uniformTypeName[] = {
		"int", "int", NULL, NULL, "vec4", "float4", "mat3", "float3x3", "mat4", "float4x4",
	};
	...
	{
	  public:
		Bin2cWriter(const bx::StringView &_name) : m_name(_name) {}
...
#undef HEX_DUMP_FORMAT
	...
	{
		if (!_keepcomments)
			return bx::strFind(_str, _find, _num);

		int32_t len = bx::min(_find.getLength(), _num);

		const char *ptr = strFindUncommented(_str.getPtr(), _str.getLength(), _find.getPtr(), len);
	...
	struct Preprocessor
	{
		Preprocessor(const char *_filePath, bool _essl, bx::WriterI *_messageWriter) : m_tagptr(m_tags), m_scratchPos(0), m_fgetsPos(0), m_messageWriter(_messageWriter), m_keepCommentsTag(NULL)
		{
		...
		void setKeepComments(bool keep) { m_keepCommentsTag->data = (void *)keep; }
	...
	void help(const char *_error = NULL)
	{
		...
		bx::printf("      --preprocess              Only pre-process.\n"
	...
	bool compileShader(const char *_varying, const char *_comment, char *_shader, uint32_t _shaderLen, const Options &_options, bx::WriterI *_shaderWriter, bx::WriterI *_messageWriter)
	{
		...
		if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL)
		{
			...
			{
				bx::snprintf(esslDefine, BX_COUNTOF(esslDefine), "BGFX_SHADER_LANGUAGE_ESSL=%d", profile->id);
			}
		}

		if (profile->lang == ShadingLang::HLSL || profile->lang == ShadingLang::Dxil)
		{
			bx::snprintf(hlslDefine, BX_COUNTOF(hlslDefine), "BGFX_SHADER_LANGUAGE_HLSL=%d", profile->id);

			if (profile->lang == ShadingLang::Dxil)
			{
				bx::snprintf(dxilDefine, BX_COUNTOF(dxilDefine), "BGFX_SHADER_LANGUAGE_DXIL=%d", profile->id);
			}
		}
		...
		else if (0 == bx::strCmpI(platform, "ios") || 0 == bx::strCmpI(platform, "osx") || 0 == bx::strCmpI(platform, "visionos"))
		{
			...
			char temp[32];
			bx::snprintf(temp, sizeof(temp), "BGFX_SHADER_LANGUAGE_METAL=%d", (profile->lang == ShadingLang::Metal) ? profile->id : 0);
		...
		while (!parse.isEmpty())
		{
			...
			const bx::StringView code = comment.isEmpty() ? line : bx::StringView(line.getPtr(), comment.getPtr());
			...
			if (eol.isEmpty())
			{
				bx::write(_messageWriter, &messageErr, "Error: Varying definition '%S' is missing a terminating ';'.\n", &trimmed);
				return false;
			}
		...
		else if ('c' == _options.shaderType) // Compute
		{
			...
			else
			{
				...
				if (preprocessor.run(input))
				{
					...
					{
						bx::write(_shaderWriter, preprocessor.m_preprocessed.c_str(), (int32_t)preprocessor.m_preprocessed.size(), &err);
			...
			else
			{
				if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL)
				{
					if (profile->lang != ShadingLang::ESSL)
					{
						...
						preprocessor.writef("#define shadow2D(_sampler, _coord) bgfxShadow2D(_sampler, _coord).x\n"
					...
					*const_cast<char *>(entry.getPtr() + 4) = '_';
					...
					else if ('v' == _options.shaderType)
					{
						...
						if (hasViewportId)
						{
							if (profile->id >= 400)
							{
								preprocessor.writef("\tuint gl_ViewportIndex : SV_ViewportArrayIndex;\n"
						...
						if (hasLayerId)
						{
							if (profile->id >= 400)
							{
								preprocessor.writef("\tuint gl_Layer : SV_RenderTargetArrayIndex;\n"
...

					{
						...
						if (profile->lang == ShadingLang::GLSL || profile->lang == ShadingLang::ESSL)
						{
							...
							if (glsl_profile < 400)
							{
								...
								if (profile->lang != ShadingLang::ESSL)
								{
									const bool need130 = (120 == glsl_profile && (false || !bx::findIdentifierMatch(input, s_130).isEmpty() || usesInterpolationQualifiers || usesTexelFetch || usesIntegerVecs));
									...
									{
										bx::stringPrintf(code, "#define texture2DProjLod  texture2DProjLodARB\n"
									...
									{
										bx::stringPrintf(code, "#define texture2DProjLod  texture2DProjLodEXT\n"
	...
	int compileShader(int _argc, const char *_argv[])
	{
		...
		else
		{
			...
			if (false // https://en.wikipedia.org/wiki/Byte_order_mark#Byte_order_marks_by_encoding
				|| '\x00' == ch || '\x0e' == ch || '\x2b' == ch || '\x84' == ch || '\xdd' == ch || '\xf7' == ch || '\xfb' == ch || '\xfe' == ch || '\xff' == ch)
			{
