...
namespace lime {
	...
	size_t OggDecoder::Decode (void* ptr, size_t frames, int byteDepth) {
		...
		while (true) {
			...
			result = ov_read ((OggVorbis_File*)handle, (char*)ptr + size, bytesWanted - size, 1, byteDepth, 1, &bitstream);
			#else
			result = ov_read ((OggVorbis_File*)handle, (char*)ptr + size, bytesWanted - size, 0, byteDepth, 1, &bitstream);
			#endif
